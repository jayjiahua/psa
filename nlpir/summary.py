...
logger = logging.getLogger('nlpir.LJSummary')


LIB_NAME = 'LJSummary'
SHORT_NAME = 'DS'

class LJSummary(BaseNlpirSDK):
    ...
    def __init__(self):
        ...
        self.SingleDoc = self.get_func('DS_SingleDoc', [c_char_p, c_double, c_int, c_bool], c_char_p)

    def single_doc(self, s, sum_rate=0.0, sum_len=250, html_tag_remove=False):
        s = self._decode(s)
        logger.debug("Get the summary of the text: %s." % s)

        result = self.SingleDoc(self._encode(s), sum_rate, sum_len, html_tag_remove)
        result = self._decode(result)

        logger.debug("The summary of the text is: %s." % result)
        return result

def test():
    summary = LJSummary()
    summary.open()
    print summary.single_doc("""
